 ...
 * Scene i is visible during [i*60, (i+1)*60], fading in over the last
...
export function renderAmbientSVG(
  ...
  <title>Contribution graph ambient animation — ${ordered.map((s) => s.name).join(" → ")}, one scene per minute</title>
  <!-- generated by contribution-splatoon ambient renderer (seed ${seed}) -->
